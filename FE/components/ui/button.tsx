...
function LoadingButton({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(
        "cursor-pointer transition-all duration-300 flex flex-row gap-2",
        buttonVariants({ variant, size, className }),
        loading && "cursor-not-allowed hover:brightness-100"
      )}
      {...props}
    >
      {loading && <CircularProgress className="border-white w-6 h-6" />}
      {props?.children}
    </Comp>
  );
}
...
export { Button, buttonVariants, LoadingButton, MotionButton };
